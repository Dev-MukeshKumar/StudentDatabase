...
object Operation2 {
  ...
  def executeOperation2(
    ...
    val marksData = getMarks(spark,logger,subjectId,classId).cache()

    import spark.implicits._
    val marksAverage = marksData.groupBy("testId").agg(functions.avg("mark").as("mark")).as[TestMarkGroup]
    val arrangedAverage = marksAverage.orderBy(functions.desc("mark")).take(3)
...
//    println("Percentage of increase in average:"+ (testAverageMarks.max - testAverageMarks.min)/100 + "%")
  ...
  private def getMarks(spark:SparkSession, logger: Logger,subjectId:Int,classId:Int): Dataset[TestMarkGroup] = {
    import spark.implicits._
