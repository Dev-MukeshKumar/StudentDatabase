...
object Operation6 {
  def executeOperation6(
    ...
    val physicsMarksGrouped = getMarksForASubject(spark,logger,classId,physicsId).cache()

    val test1Marks = getMarksForATest(spark,logger,classId,1,groupIds).cache()

    import spark.implicits._
    val physicsOverallAverage = physicsMarksGrouped.map(data => data match {
      case Mark(Some(groupId), Some(classId), Some(subjectId), Some(studentId), Some(marks)) => StudentAverage(studentId, marks.values.map(value => value.getOrElse(0.0)).sum / data.test_marks.get.size)
    }).as[StudentAverage].filter(data => data.average >50)

    val test1OverallAverageMarks = test1Marks.groupBy("student_id").agg(avg("mark").as("average")).as[StudentAverage].filter(data => data.average > 50)

    val graceMarkEligibleStudents = physicsOverallAverage.join(test1OverallAverageMarks,physicsOverallAverage.col("student_id")===test1OverallAverageMarks.col("student_id"),"inner").as[(Int,Double,Int,Double)].collect().toList

    val graceMarkEligibleStudentsIdList = graceMarkEligibleStudents.map(data => data._1)
    ...
    val classXIIPhysicsPassPercentage = Operation3.calculatePassPercentage(spark,logger,classId.toString).where(s"subject_id = $physicsId")
  ...
  def getMarksForASubject(spark: SparkSession, logger: Logger, classId: Int,subjectId:Int): Dataset[Mark] = {
    import spark.implicits._
    spark.read.format("org.apache.spark.sql.cassandra")
      .options(Map("keyspace" -> "assignment2", "table" -> "marks"))
      .load
      .where(s"class_id = ${classId} and subject_id=${subjectId}")
      .as[Mark]
  }
  ...
  private def getStudentsName(spark:SparkSession,logger: Logger,classId:Int,groupIds:String,studentsIds:List[Int]):List[String] = {
    ...
    val readData = spark.read.format("org.apache.spark.sql.cassandra")
      ...
      .load
      .where(s"class_id=$classId and group_id in ($groupIds)")
      .as[Student]
